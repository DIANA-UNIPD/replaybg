...
class Vettoretti19CGM(Sensor):
    """A CGM sensor based on the error model in Vettoretti et al., Sensors, 2019.

    Concrete :class:`~sensors.sensor.Sensor` whose measurement is a continuous glucose
    monitoring (CGM) reading of the model output (interstitial glucose). The error
    model adds a time-varying calibration error plus colored noise generated by an
    AR(2) process whose parameters are sampled per sensor connection.

    Attributes:
        cgm_error_parameters: Array of the sampled CGM error-model parameters.
        output_noise_sd: Output standard deviation of the CGM error model (mg/dl).
        ekm1: Memory term for the colored noise, e(k-1).
        ekm2: Memory term for the colored noise, e(k-2).
    """

    def __init__(self):
        """Constructs the CGM sensor. The sample time ``ts`` (5) is inherited."""
        # Declare the CGM-specific attributes before connecting: ``super().__init__``
        # calls ``connect_new``, which populates them with a sampled error realization.
    ...
    def connect_new(self, connected_at=0):
        ...
        """
        super().connect_new(connected_at)
        ...
        # Modulation factor of the covariance of the parameter vector not to generate
        # too extreme realizations of parameter vector
        f = 0.90
    ...
    def measure(self, value, past_values, t):
        """Provide a CGM measure using the model of Vettoretti et al., Sensors, 2019.

        Args:
            value: The current interstitial glucose concentration (mg/dl).
            past_values: The series of past interstitial glucose concentrations (mg/dl).
            t: Current time, in days from when the sensor was connected.

        Returns:
            The CGM measurement at the current time (mg/dl).
        """
        # Apply calibration error
        ig_s = (self.cgm_error_parameters[0] + self.cgm_error_parameters[1] * (t + self.t_offset) +
                self.cgm_error_parameters[2] * ((t + self.t_offset) ** 2)) * value + self.cgm_error_parameters[3]
